...
public class DamagedTests
{
    ...
    {
        var e = await Assert.ThrowsAsync<BdfIllegalWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "not_a_bdf.bdf")));
        Assert.Equal("This", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfParseException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "not_support_version.bdf")));
        Assert.Equal("Spec version not support: 2.2", e.Message);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_font.bdf")));
        Assert.Equal("FONT", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_size.bdf")));
        Assert.Equal("SIZE", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_fontboundingbox.bdf")));
        Assert.Equal("FONTBOUNDINGBOX", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_end_properties.bdf")));
        Assert.Equal("ENDPROPERTIES", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_chars.bdf")));
        Assert.Equal("CHARS", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_encoding.bdf")));
        Assert.Equal("ENCODING", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_swidth.bdf")));
        Assert.Equal("SWIDTH", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_dwidth.bdf")));
        Assert.Equal("DWIDTH", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_bbx.bdf")));
        Assert.Equal("BBX", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<FormatException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_end_char.bdf")));
        Assert.Equal("Could not find any recognizable digits.", e.Message);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfMissingWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "no_line_end_font.bdf")));
        Assert.Equal("ENDFONT", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfIllegalWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "illegal_word_in_font.bdf")));
        Assert.Equal("ABC", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfIllegalWordException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "illegal_word_in_char.bdf")));
        Assert.Equal("DEF", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfCountException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "incorrect_properties_count.bdf")));
        Assert.Equal("STARTPROPERTIES", e.Word);
    }
    ...
    {
        var e = await Assert.ThrowsAsync<BdfCountException>(async () => await BdfFont.LoadAsync(Path.Combine("assets", "damaged", "incorrect_chars_count.bdf")));
        Assert.Equal("CHARS", e.Word);
    }
}
